...
class UsuarioDescarga(threading.Thread):
    def __init__(self, id_solicitud, servidor):
        ...
        self.max_intentos = 3
        # Nombrar el hilo como "Usuario X" para claridad en logs
    ...
    def run(self):
        ...
        while intentos < self.max_intentos and not conectado:
            if self.servidor.solicitar_conexion():
                ...
                intentos += 1
                print(f"[!] Servidor lleno. {self.name} en espera... (Intento {intentos}/{self.max_intentos})")
        ...
        if conectado:
            ...
            print(f"[+] {self.name} ACEPTADO. Descargando {self.tamano_archivo} MB. (Tiempo estimado: {tiempo_estimado_segundos:.2f}s)")
            bloques = 3
            tiempo_por_bloque = tiempo_estimado_segundos / bloques
            
            for i in range(bloques):
                time.sleep(tiempo_por_bloque)
                print(f"[Descargando] {self.name}: {int(((i+1)/bloques)*100)}%")

            self.servidor.registrar_salida(self.id_solicitud, self.tamano_archivo)
            print(f"[-] {self.name} COMPLETADO. Cupo liberado.")
        
        else:
            print(f"[x] {self.name} CANCELADO por Timeout: No se logró conseguir cupo.")